...
class Unet(object):
    ...
    def __init__(self, channels=1, n_class=1, cost="dice_coefficient", predict_thresh=0.5, cost_kwargs={}, **kwargs):
        ...
        self.x = tf.placeholder(tf.float32, shape=[3, 148, 148, 148, channels], name='features_placeholder')
        self.y = tf.placeholder(tf.float32, shape=[3, 60, 60, 60, n_class], name='labels_placeholder')
        self.keep_prob = 0.75
        ...
        logging.info("Actual Output Shape: {}".format(self.logits.get_shape()))
        ...
        self.cross_entropy = tf.reduce_mean(
            cross_entropy(tf.reshape(self.y, [-1, n_class], name='cross_entro_label_reshape'),
                          tf.reshape(pixel_wise_softmax_2(self.logits), [-1, n_class], name='px_logit_reshape')))

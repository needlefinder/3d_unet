...
def _load_file(path, dtype=np.float32, padding=None):
        ...
        ata = np.load(path).astype(dtype)
        return data

def load_data(min_iter=np.inf):
    ...
    data, target = [], []
    for i in trange(min(min_iter,len(files_training))):
...
class Trainer(object):
    ...
    def __init__(self, batch_size=1, optimizer="momentum", opt_kwargs={}):
    ...
    def train(self, output_path, training_array=None, validation_array=None, testing_array=None, \
        ...
        with tf.Session() as sess:
            ...
            correct_pred = tf.cast(
                tf.equal(tf.reshape(predicter_label, [-1, n_class]), tf.reshape(next_label, [-1, n_class])), tf.float32)
            accuracy = tf.reduce_mean(correct_pred)
            loss = _get_cost(predicter, next_label)
            self.cost = loss
            
            training_op = tf.train.AdamOptimizer(learning_rate=tf.Variable(0.001)).minimize(loss, global_step=tf.Variable(0))
            ...
            for epoch in range(epochs):
                ...
                for step in range((epoch * training_iters), ((epoch + 1) * training_iters)):
                    loss_val, _ = sess.run([self.cost,training_op])
                    total_loss += loss_val
                    logging.info("Iter {:}, Minibatch Loss= {:.4f}, Accuracy= {:.4f}".format(step, loss.eval(), accuracy.eval()))

                    if step % 10 == 0:
                        self.output_minibatch_stats(sess, summary_writer, step)

                self.output_epoch_stats(epoch, total_loss, training_iters, 0.001)
...
def predict(data, model_path):
    ...
    arr_pred = predict_full_volume2(arr_data, model_path=model_path)
    print(np.array(data).shape, np.array(arr_pred).shape)
    print("Merging subvolumes")
    full_pred = recombine(arr_out=arr_pred, data=data)
    return full_pred
...
def predict_multiple2(features, model_path):
    """

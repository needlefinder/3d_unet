...
class Unet(object):
    ...
    def __init__(self, channels=1, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        self.predicter_label = tf.cast(tf.argmax(tf.reshape(self.predicter, [-1, n_class]),axis=1), tf.float32)
        self.correct_pred = tf.cast(tf.equal(tf.argmax(tf.reshape(self.predicter, [-1, n_class]),axis=1), tf.argmax(tf.reshape(self.y, [-1, n_class]),axis=1)), tf.float32)
        self.cost = self._get_cost(self.logits, self.predicter, self.predicter_label, cost, cost_kwargs)
    ...
    def _get_cost(self, logits, predicter, predicter_label, cost_name, cost_kwargs):
        """
        ...
        with tf.name_scope('cost_function'):
            ...
            flat_predicter_label = tf.reshape(predicter_label, [-1, self.n_class], name='flat_predicter_label_reshape')
...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_array=None, validation_array=None, testing_array=None,              training_iters=10, epochs=100, dropout=0.75, display_step=1, restore=False):
        """
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(epochs):
                ...
                for step in range((epoch*training_iters), ((epoch+1)*training_iters)):
                    ...
                    total_loss += loss                    
                    logging.info("Iter {:}, Minibatch Loss= {:.4f}, Accuracy= {:.4f}, Prediction= {}, Desired= {}".format(step, loss, acc, np.unique(prediction_labels, return_counts=True), np.unique(y[..., 1], return_counts=True)))
    ...
    def output_minibatch_stats(self, sess, summary_writer, step, batch_x, batch_y):
        print(batch_x.shape)
        print(batch_y.shape)
        summary_str, loss, acc, prediction_labels = sess.run([self.summary_op, 
                                                        self.net.cost, 
                                                        self.net.accuracy,
